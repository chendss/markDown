...
// 加载样式
import styles from './UploadLess.less';
// 引入网站配置
...
// 引入上传计算文件名方法
import uploadCfg from 'utils/upload';
const { calculate_object_name } = uploadCfg;
// 样式管理
...
export default class extends Component {
  ...
  getSign(file){
    ...
    return new Promise((resolve, reject) => {
      $.ajax({
        url: apis.getPolicyUrl,
        type: "GET",
        headers: {
          auth: Cookie.get(config.cookie.auth_name)
        },
        success: (res) => {
          // 更新OSS上传地址等到状态机
        ...
        error: (err) => {
          Modal.error({
            title: '系统提示',
            content: '获取oss签名失败' + JSON.stringify(err)
          })
          reject(err)
        }
      });
  ...
  render(ReactElement, DOMElement, callback) {
    ...
    const getImgSrc = () => {
      if(this.state.pictureSrc){
        let url = this.state.pictureSrc;
        if(this.state.imgUrlSize){
          url = url + this.state.imgUrlSize;
        }else{
          if(config.imgSize){
            url = url + config.imgSize;
          }
        }
        return url;
      }else{
        return config.uploadImgdefault || '';
      }
    }
            ...
            <div>
              {
                ...
                <img src={getImgSrc()} alt="" className={styles.logoImg}/>
                </p>
              }
